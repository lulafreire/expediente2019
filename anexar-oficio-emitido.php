    ...
    <div class="border-bottom py-2">
        <?php
            ...
            while($o=mysqli_fetch_array($sqlOficio)) {
                ...
                $idEmissor = $o['emissor'];
                $idDest    = $o['destinatario'];


                // Pesquisa os dados do destinatario
                $sqlDest = mysqli_query($conn, "SELECT * FROM contatos WHERE id = '$idDest'");
                while($e=mysqli_fetch_array($sqlDest)) {

                    $nomeDest  = utf8_encode($e['nome']);
                    $cargoDest = utf8_encode($e['cargo']);
                    $orgaoDest = utf8_encode($e['orgao']);
                }
            }
            
            echo "
            Ofício nº <b>$numero/$ano/$siglaUnidade/INSS</b> de $dtEmissao<br>
            Destinatário: <b>$nomeDest</b> ($cargoDest - $orgaoDest)<br>";
