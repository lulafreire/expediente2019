...
if ($_UP['tamanho'] < $_FILES['arquivo']['size'] or $_FILES['arquivo']['size'] == 0) {
    ...
    $gravaAnexo = mysqli_query($conn,"insert into anexos (arquivo, tam, descricao, referencia, data) values ('$novoNome','$tamMb','$descricao','$id_oficio', curdate())");
    $gravaEventoAnexo = mysqli_query($conn,"insert into eventos (data, descricao, referencia) values (now(),'NOVO ANEXO ENVIADO','$id_oficio')");
            
}

header('location: anexar-oficio-recebido.php?id='.$id_oficio);
