...
<div class='row mt-3' style="min-height: 375px;">
    <div class="col-12">
        ...
        $emitidos = mysqli_query($conn, "SELECT * from documentos where tipo = '0' AND unidade = '$codUnidade' ORDER by id DESC");
        ...
        $res_emitidos = mysqli_query($conn, "SELECT * from documentos where tipo = '0' AND unidade = '$codUnidade' ORDER by id DESC LIMIT $inicio, $oficiosPorPagina");
        while($r=mysqli_fetch_array($res_emitidos)) {
            ...
            <tr>
                ...
                <td><a href='excluir-oficio.php?id=$id&location=sql_emitidos&pagina=$pagina' target='_parent' class='text-dark'><i class='far fa-trash-alt' title='Excluir'></i></a></td>
            ...
            <div class='modal fade' id='detalhaOficio$id' tabindex='-1' role='dialog' aria-labelledby='exampleModalLabel' aria-hidden='true'>
                <div class='modal-dialog modal-xl modal-dialog-centered' role='document'>
                    ...
                    <div class='modal-header'>
                        <h5 class='modal-title' id='exampleModalLabel'>Ofício <strong>$numero/$anoEmissao/$siglaUnidade/INSS</strong></h5>
            ...
            if($resResp !='') {
            // Modal Detalha Ofício Referencia
            echo "
            <div class='modal fade' id='detalhaReferencia$idOficioOriginal' tabindex='-2' role='dialog' aria-labelledby='exampleModalLabel' aria-hidden='true'>
                <div class='modal-dialog modal-dialog-centered' role='document'>
                    <div class='modal-content'>
                    <div class='modal-header'>
                        <h5 class='modal-title' id='exampleModalLabel'>Ofício <strong>$numOficioOriginal/$anoEmissaoOficioOriginal</strong></h5>
                        <button type='button' class='close' data-dismiss='modal' aria-label='Fechar'>
                        <span aria-hidden='true'>&times;</span>
                        </button>
                    </div>
                    <div class='modal-body'>
                        <strong>Emissor:</strong> $nomeEmissorOficioOriginal<br>
                        <strong>Cargo:</strong> $cargoEmissorOficioOriginal<br>
                        <strong>Órgão:</strong> $orgaoEmissorOficioOriginal<br>
                        <strong>Interessado:</strong> $interessado<br>
                        <strong>Emitido em:</strong> $dtEmissaoOficioOriginal<br> 
                        <strong>Prazo:</strong> $dtPrazoOficioOriginal<br>
                        
                        <hr>
                        <strong>Resumo:</strong> $resumoOficioOriginal
                        <hr>
                        <strong>ANEXOS:</strong><br>";

                        // Pesquisa os anexos relacionados a este ofício
                        $sqlAnexos = mysqli_query($conn, "SELECT * FROM anexos WHERE referencia ='$idOficioOriginal' ORDER BY id ASC");
                        while($a=mysqli_fetch_array($sqlAnexos)) {
                            $idAnexo = $a['id'];
                            $arquivo = $a['arquivo'];
                            $tam     = $a['tam'];
                        ...
                        $sqlEventos = mysqli_query($conn, "SELECT * FROM eventos WHERE referencia = '$idOficioOriginal' ORDER BY id ASC");
